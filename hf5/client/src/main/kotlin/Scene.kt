...
//        gameObjects.add(animatedObject)
    ...
    val tentacleProgram = Program(gl, vsTrafo, fsTextured)
    val tentacleMaterial = Material(tentacleProgram).apply {
        this["colorTexture"]?.set(
            Texture2D(gl, "media/Color.jpg")
        )
    }


    val tentacleGeometry = TentacleGeometry(gl, 10f, 2f, 16)
    val tentacleMesh = Mesh(tentacleMaterial, tentacleGeometry)

    init {
        val animatedObject = GameObject(tentacleMesh)
        gameObjects.add(animatedObject)
    }
...
//    val skeletonTexture = DataTexture(
//        gl,
//        Uint32Array(animation.skeleton.buffer),
//        zombieGeom.nBones,
//        1,
//        GL.RGBA32UI,
//        GL.RGBA_INTEGER,
