...
Mesh::GeometryP Mesh::GeometryLoader::createGeometryFromFile(ID3D11Device* device, const char* filename)
{
    // Open the file
	...
	Mesh::GeometryP geometry = createGeometryFromMemory(  device,
                                meshData,
                                cBytes);
	delete [] meshData;
    return geometry;
...
Mesh::GeometryP Mesh::GeometryLoader::createGeometryFromMemory(ID3D11Device* device, BYTE* data, unsigned int nBytes)
{	
	BYTE* p = data;
	...
	if(gbsVersion == 0x1) // barebone .gbs
	{
		...
		while(*p)
		{
			...
			size_t semanticNameLength = strlen( (char*)p) + 1;
		...
		Mesh::GeometryP indexed = Mesh::Indexed::createFromSingleStream(device, indexedDesc, vertexStream);

		for(uint iElements=0; iElements<nElements; iElements++)
